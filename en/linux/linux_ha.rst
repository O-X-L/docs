...
If you want to use encryption - you have to create a key-file and place it at :code:`/etc/corosync/keyfile` on all cluster nodes.
...
Make sure the service only runs on one node, but should always be on the same node as the floating IP:
...
.. code-block::
    ...
    location loc resServiceIP role=Started inf: node1
...
Here is an example script that can be used to monitor your LinuxHA cluster-status:
